...
public class recycleGalleryShoeAdapter extends RecyclerView.Adapter<recycleGalleryShoeAdapter.galleryViewHolder> {
    private ArrayList<CustomImage> customImages = new ArrayList<>();
    private Context context;

    public recycleGalleryShoeAdapter(Context context, ArrayList<CustomImage> customImages ){
    ...
    public void onBindViewHolder(galleryViewHolder holder, int position) {
        ...
        if(position==0){
            Glide.with(context).load(R.drawable.x22)
                    .asBitmap()
                    .placeholder(R.drawable.ic_launcher)
                    .diskCacheStrategy(DiskCacheStrategy.SOURCE)
                    .into(holder.imageView);
        }else if(position==1){
            Glide.with(context).load(R.drawable.x33)
                    .asBitmap()
                    .placeholder(R.drawable.ic_launcher)
                    .diskCacheStrategy(DiskCacheStrategy.SOURCE)
                    .into(holder.imageView);
        }else if(position==2){
            Glide.with(context).load(R.drawable.x4)
                    .asBitmap()
                    .placeholder(R.drawable.ic_launcher)
                    .diskCacheStrategy(DiskCacheStrategy.SOURCE)
                    .into(holder.imageView);
